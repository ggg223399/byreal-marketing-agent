...
import { mergeFile } from '../../skills-engine/merge.js';

type FileInputHashes = {
  base: string;
  current: string;
  skill: string;
};
...
function setupRerereAdapter(_relPath: string, _base: string, _ours: string, _theirs: string): void {
}

function cleanupMergeState(relPath: string): void {
  execSync(`git checkout -- "${relPath}"`, { stdio: 'pipe', cwd: projectRoot });
}
