...
```
byreal-marketing-agent/          # NanoClaw Skill 包
├── SKILL.md                     # 安装指南（Phases 格式）
├── manifest.yaml                # Skill 元数据（adds/modifies/deps）
├── README.md                    # 项目文档
├── add/                         # 新增到 NanoClaw 的文件
│   ├── marketing-agent/         # 业务逻辑
│   │   ├── collector/           # 数据采集（adapters: mock, twitterapi_io, twitter_v2, xai_search）
...
└── modify/                      # 三向合并到 NanoClaw 现有文件
    ├── src/channels/discord.ts  # 营销版 Discord handler
    └── vitest.config.ts         # 测试路径配置
```

## 安装

### 快速安装（NanoClaw Skill Engine）

```bash
cd /path/to/your/nanoclaw
git clone https://github.com/ggg223399/byreal-marketing-agent .claude/skills/add-marketing-agent
npx tsx scripts/apply-skill.ts .claude/skills/add-marketing-agent
```

Skill engine 自动完成：
- 复制业务逻辑到 `marketing-agent/` 目录
- 三向合并 Discord handler 和测试配置
- 安装 npm 依赖（better-sqlite3, yaml）
- 运行数据库迁移
- 执行测试验证

安装后按 SKILL.md Phase 3-5 完成 Discord 频道配置、环境变量和 cron 设置。

### 前置条件

- NanoClaw 已部署并运行
- Discord skill 已安装（运行过 `/add-discord`）
- 数据源 API key（xAI / TwitterAPI.io / Twitter v2，或用 mock 测试）
