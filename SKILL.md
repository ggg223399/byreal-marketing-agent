...
1. 在 Discord Server 里创建 5 个频道（如果还没创建）：
   - `#risk-alerts`
   - `#opportunities`
   - `#ecosystem-feed`
   - `#draft-review`
...
- `notifications.draft_webhook_url`：#draft-review webhook
...
| `draft reply #N` | 展示信号摘要 + 4 个语气按钮，点击后生成对应语气的回复草稿 |
| `reject #N` | 拒绝信号 |
| `show config` | 查看当前配置 |
| `status` | 系统统计 |
...
| 按钮 | 语气说明 |
|------|---------|
| 🧑‍💼 Helpful Expert | 专业权威，提供具体价值 |
| 👋 Friendly Peer | 轻松对等，亲切友好 |
| 🙏 Humble Ack | 感恩致谢，不强推 |
| 💬 Direct Rebuttal | 正面回应关切，建设性反驳 |

点击语气按钮后，Bot 生成该语气的回复草稿。点击 🔄 Refresh 可重新生成。
## 自定义
