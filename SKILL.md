...
3. Discord server 已创建以下频道：
   - `#marketing-bot`（机器人交互，注册为 NanoClaw group）
   - `#needs-reply`（需要回复的信号）
   - `#needs-interaction`（需要互动的信号）
   - `#tier1-signals`（Tier 1 高优先级）
   - `#tier2-signals`（Tier 2 中优先级）
   - `#tier3-signals`（Tier 3 低优先级）
   - `#noise`（低价值信号）
   - `#periodic-summary`（定时摘要）
   - `#draft`（草稿发布）
   - `#daily-digest`（需要 Webhook，用于 cron 推送日报）
...
5. 左侧菜单 → 「OAuth2」→ 「URL Generator」
   - Scopes 勾选 `bot`
   - Bot Permissions 勾选 `Send Messages`、`Read Message History`、`View Channels`、`Manage Messages`
   - 复制生成的 URL，在浏览器打开，选择你的 Server，授权

### 准备 2：创建 Discord 频道

1. 在 Discord Server 里创建以下频道（如果还没创建）：
   - `#needs-reply`、`#needs-interaction`
   - `#tier1-signals`、`#tier2-signals`、`#tier3-signals`
   - `#noise`、`#periodic-summary`、`#draft`
   - `#daily-digest`
2. 仅 `#daily-digest` 需要 Webhook：右键频道名 → 「Edit Channel」→ 「Integrations」→ 「Webhooks」→ 「New Webhook」
3. 复制 Webhook URL，后面填入 `config.yaml` 的 `digest_webhook_url`
...
- `notifications.needs_reply_channel`：需要回复的信号频道名（默认 needs-reply）
- `notifications.needs_interaction_channel`：需要互动的信号频道名（默认 needs-interaction）
- `notifications.tier1_channel` ~ `tier3_channel`：Tier 频道名
- `notifications.noise_channel`：噪音频道名（默认 noise）
- `notifications.summary_channel`：摘要频道名（默认 periodic-summary）
- `notifications.draft_channel`：草稿发布频道名（默认 draft）
