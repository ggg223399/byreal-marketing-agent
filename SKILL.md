...
将本仓库所有文件复制到 NanoClaw 根目录：

```bash
# 在 NanoClaw 根目录下执行
cp -r collector classifier generator approval notifications digest governance lib types db config prompts tests scripts groups ./
mkdir -p src/channels
cp src/channels/discord.ts src/channels/discord.ts
